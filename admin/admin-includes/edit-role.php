...
// if(isset($_SESSION['username']) && isset($_SESSION['role']) && ($_SESSION['role']==='admin' || $_SESSION['role'] === 'Owner')){
    ...
    if(isset($_POST['perm-crm-edit-proforms'])){
        array_push($permissions,"crm edit proforms");
    }
    if(isset($_POST['perm-crm-generate-proforms'])){
        array_push($permissions,"crm generate proforms");
    }
    if(isset($_POST['perm-crm-aprove-proforms'])){
        array_push($permissions,"crm aprove proforms");
    }
    ...
    $sql = "UPDATE `admin_roles` SET `role`='$roleName',`permissions`='$permissionsJSON',`departament`='$roleDepartment' WHERE `roleId`='$roleId'";
