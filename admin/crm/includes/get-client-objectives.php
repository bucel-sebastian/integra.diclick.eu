...
error_reporting(0);
    ...
    if(isset($_GET['id'])){
        ...
        $sql = "SELECT * FROM `obiective` WHERE `unicId`='$clientId' AND `tip`='1' ORDER BY `perioada_sfarsit` DESC";
        $result = mysqli_query($crmConn,$sql);
        if($result){
            if(mysqli_num_rows($result)>0){
                while($row=mysqli_fetch_assoc($result)){
                    ...
                    echo ',{"id":"'.$row['id'].'","clientId":"'.$row['unicId'].'","tip":"'.$row['tip'].'","clientObj":"'.$row['client_obj'].'","socialObj":"'.$row['social_obj'].'","performanceObj":"'.$row['performance_obj'].'","dezvoltareOnlineObj":"'.$row['dezvoltare_online_obj'].'","marketingObj":"'.$row['marketing_obj'].'","perStart":"'.$row['perioada_start'].'","perSfarsit":"'.$row['perioada_sfarsit'].'","id":"'.$row['id'].'","comentarii":"'.$text.'"}';
