...
$invoiceNumber=$_POST['edit-proforma-invoice-number'];

$sql =  "SELECT * FROM `proforme` WHERE `invoice_number`='$invoiceNumber'";
$result = mysqli_query($crmConn,$sql);
if($result){
    if(mysqli_num_rows($result)===1){
        $row = mysqli_fetch_assoc($result);
        $beforeClient = $row['client'];
    }
}

$months = array("Ian","Feb","Mar","Apr","Mai","Iun","Iul","Aug","Sep","Oct","Noi","Dec");

$emitor=$_POST['edit-proforma-emitator'];
$client=$_POST['edit-proforma-client'];
$totalDePlata=$_POST['total-de-plata'];
$totalFTva=$_POST['total-fara-tva'];
$totalTva=$_POST['total-tva'];

$moneda = $_POST['edit-proforma-moneda'];

if($moneda === "EURO-ECHIVALENT"){
    $cursEuro=$_POST['edit-proforma-curs-euro'];
...
$delegat = $_POST['edit-proforma-pers-delegata'];
...
for($i=0;$i<sizeof($_POST['product-name']);$i++){
    ...
    $invoiceTableData.='<tr>
        ...
        <td style="border:2px solid #000;padding: 2px 5px;border-collapse:collapse;">'.$_POST['product-um'][$i].'</td>
        <td style="border:2px solid #000;padding: 2px 5px;border-collapse:collapse;">'.$_POST['product-cant'][$i].'</td>
        <td style="border:2px solid #000;padding: 2px 5px;border-collapse:collapse;">'.$_POST['product-price'][$i].' '.$euroSymbol.'</td>
        <td style="border:2px solid #000;padding: 2px 5px;border-collapse:collapse;">'.$_POST['product-val'][$i].' '.$euroSymbol.'</td>
        <td style="border:2px solid #000;padding: 2px 5px;border-collapse:collapse;">'.$_POST['product-tva'][$i].' '.$euroSymbol.'</td>
...
$invoiceVAT=$_POST['edit-proforma-vat'];
$invoicePayDay=$_POST['edit-proforma-tp'];
$totalWithoutVAT=$_POST['total-fara-tva'];
$totalVAT=$_POST['total-tva'];
$invoiceTotal=$_POST['total-de-plata'];


$sql="UPDATE `proforme` SET `client`='$client',`nr_reg_comert`='".$row['nr_reg_comert']."',`cod_fiscal`='".$row['cod_fiscal']."',`adresa_facturare`='".$row['adresa_facturare']."',`oras_facturare`='".$row['oras_facturare']."',`judet_facturare`='".$row['judet_facturare']."',`tara_facturare`='".$row['tara_facturare']."',`vat`='$invoiceVAT',`produse`='$productsListJson',`termen_plata`='$invoicePayDay',`emitor`='$emitor',`total_tva`='$totalTva',`total_f_tva`='$totalFTva',`total_de_plata`='$totalDePlata',`status`='1' WHERE `invoice_number`='$invoiceNumber'";

// "UPDATE `proforme` SET `client`='$client',`nr_reg_comert`='".$row['nr_reg_comert']."',`cod_fiscal`='".$row['cod_fiscal']."',`adresa_facturare`='".$row['adresa_facturare']."',`oras_facturare`='',`judet_facturare`='',`tara_facturare`='',`data_factura`='',`invoice_number`='',`vat`='',`produse`=',`termen_plata`='',`emitor`='',`total_tva`='',`total_f_tva`='',`total_de_plata`='',`invoice_created`='',`created_by`='',`status`='1',`persoana_delegata`='',`moneda`='',`curs_euro`='' WHERE `invoice_number`='$invoiceNumber'";
if(!mysqli_query($crmConn,$sql)){
    echo "fail ".$crmConn->error;
}
...
$dompdf -> render();
if(unlink('../../../clients-resources/'.$beforeClient.'/proforms/'.$invoiceNumber.'.pdf')){
    ...
    echo '{"status":"fail","error":"file unlink error"}';
}
