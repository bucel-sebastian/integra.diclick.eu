...
if(isset($_POST['clientId']) && isset($_POST['number']) && isset($_POST['proforma-mail-subject']) && isset($_POST['proforma-mail-body'])){
    

        $client = $_POST['clientId'];
        $file = $_POST['number'];
        $subject = $_POST['proforma-mail-subject'];
        $bodyText = $_POST['proforma-mail-body'];
        $wishes = $_POST['proforma-mail-wishes'];

        $sender = $_SESSION['user-data']->unicId;
        $sql = "SELECT * FROM `admin_users` WHERE `unicId`='$sender'";
        $result=mysqli_query($conn,$sql);
        if($result){
            if(mysqli_num_rows($result)===1){
                $row = mysqli_fetch_assoc($result);
                $senderName = $row['prenume']." ".$row['nume'];//
                $senderPhone = $row['telefon']; 
                $senderMail = $row['email'];
                $senderFunction = strtoupper($row['functie']);
            }
            else{
                echo "nu intra";
            }
        ...
        $toMails = array();
        $attachFile= '../../../clients-resources/'.$client.'/proforms/'.$file.'.pdf';
