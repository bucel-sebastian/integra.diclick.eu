        ...
        while($row = mysqli_fetch_assoc($result)){
            ...
            for($i=0;$i<=$numFiles;$i++){
                ...
                if(strpos($realFile[$i],'.pdf')){
                    $out.= "<div><a class='updates-download-link' href='".$realFile[$i]."' download><i class='fas fa-file-pdf'></i></a><span>".$fileName[$i]."</span></div>";
                }
                else if(strpos($realFile[$i],'.png')||strpos($realFile[$i],'.jpeg') || strpos($realFile[$i],'.jpg')){
                    $out.="<div><a class='updates-download-link' href='".$realFile[$i]."' download><i class='fas fa-file-image'></i></a><span>".$fileName[$i]."</span></div>";    
                }
                else if(strpos($realFile[$i],'.xlsx')){
                    $out.="<div><a class='updates-download-link' href='".$realFile[$i]."' download><i class='fas fa-file-excel'></i></a><span>".$fileName[$i]."</span></div>";
                }
                else if($realFile[$i]!=""){
