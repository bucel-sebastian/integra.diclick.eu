...
for($i=0;$i<sizeof($_POST['product-name']);$i++){
    ...
    $invoiceTableData.='<tr>
        ...
        <td style="border:2px solid #000;padding: 2px 5px;border-collapse:collapse;">'.$_POST['product-price'][$i].'</td>
        <td style="border:2px solid #000;padding: 2px 5px;border-collapse:collapse;">'.$_POST['product-val'][$i].'</td>
        <td style="border:2px solid #000;padding: 2px 5px;border-collapse:collapse;">'.$_POST['product-tva'][$i].'</td>
...
$sql="INSERT INTO `proforme`(`client`, `nr_reg_comert`, `cod_fiscal`, `adresa_facturare`, `oras_facturare`, `judet_facturare`, `tara_facturare`, `data_factura`, `invoice_number`, `vat`, `produse`, `termen_plata`, `emitor`,`total_tva`,`total_f_tva`,`total_de_plata`,`created_by`) VALUES ('$client','".$row['cod_fiscal']."','".$row['nr_reg_comert']."','".$row['adresa_facturare']."','".$row['oras_facturare']."','".$row['judet_facturare']."','".$row['tara_facturare']."','".date("Y-m-d")."','$invoiceNumber','$invoiceVAT','$productsListJson','$invoicePayDay','$emitor','$totalTva','$totalFTva','$totalDePlata','$adminId')";
